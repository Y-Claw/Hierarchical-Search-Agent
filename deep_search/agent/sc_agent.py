...
from deep_search.agent.prompts.agent_prompting import get_full_system_prompt, OPENAI_PROMPT, TOOL_SYSTEM_PROMPT, MOCK_QWQ_PROMPT
...
class SelfConsistencyAgent:
    ...
    def __init__(self, model: str, toolcall_model: str=None, agent_work_dir: str=None, mcp_server_name: str=None, max_steps: int=16, 
        ...
        self.sc_num = kwargs.get("sc_num", 3)
    ...
    def _generate_tool_calls(self, messages: list[dict], thinking: str) -> list[dict]:
        messages = [{"role": "system", "content": TOOL_SYSTEM_PROMPT}] + messages[1:] + [{"role": "user", "content": thinking}]
        try:
...
async def main():
    ...
    action_agent = SelfConsistencyAgent(model="Qwen2.5-32B-Instruct-128k")
    ...
    try:
        done, action_history = await action_agent(query="The arena where the Lewiston Maineiacs played their home games can seat how many people?", fact=None)
        print(done)
