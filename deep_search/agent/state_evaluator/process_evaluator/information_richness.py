...
class InformationRichnessEvaluator:
...
{
    "reason": "The search trajectory is insufficiently relevant, lacking additional insights that could enhance the understanding of the initial query.",
    "level": 1
}
        """
...
"""
    ...
    def evaluate_pair(self, query, msg_1, msg_2):
        system_message = """You are a helpful assistant tasked with evaluating the richness of information in search histories. You need to compare two search histories to determine which one contains more information-rich content relevant to the query. There are 5 levels of information richness:
1. Excellent richness; The search history is fully relevant to the query, providing comprehensive and detailed information from multiple sources that thoroughly addresses the query.
2. Good richness; The search history is fully relevant to the query and provides key information that adequately addresses the query.
3. Moderate richness; Most of the search steps are relevant to the query, but the information may lack depth or include some irrelevant details.
4. Limited richness; Some search steps are relevant to the query, but the information is sparse or the search history deviates from the query.
5. Insufficient richness; Most of the search steps are not relevant to the query, and the information provided is minimal or off-topic.

Please evaluate which search history is more information-rich. You should think step by step and provide your reasoning. return the result in the following format:
{
    "reason": "The search history 1 contains ..., but the search history 2 does not contain ... . So the search history 1 is more information-rich.",
    "better_history": 1 / 2
}
