...
from deep_search.utils.api import call_openai, call_deploy
...
class HotpotQAEvaluator:
    ...
    async def get_answer(self, task_id, query, ground_truth):
        messages = [
            {"role": "system", "content": """You are an AI model designed to answer user questions accurately and concisely. Think step by step first, then provide a clear and informative responses to the user's question. You should provide the answer in JSON format as follows:
{
    "think": "your think",
    "content": "your answer"
}"""}, 
        ...
        if self.model_name == "gpt-4o":
            response = call_openai(model="gpt-4o-2024-11-20", messages=messages)['content']
        else:
            response = call_deploy(model="QwQ-32B", messages=messages)['content']
    ...
    @retry(3,1)
    def judger(self, question: str, answer: str, standard_answer: str, repeat_times=3):
        judge_result = []
...
Based on the given question and corresponding standard answer, determine if the user's answer matches the standard answer (whether it correctly answers the question). Return in JSON format.
    ...
    """
            ...
            if is_correct in [1, "1", True, "true"]:
                judge_result.append(True)
            else:
                judge_result.append(False)
        return sum(judge_result) / len(judge_result)
    ...
    async def _evaluate_sample(self, sample):
        ...
        if task_id in self.results:
            ...
            is_correct = self.judger(sample['problem'], self.results[task_id]['response'], " / ".join(sample['answer']))
