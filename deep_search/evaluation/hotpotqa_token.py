...
Based on the given question and corresponding standard answer, determine if the user's answer matches the standard answer (whether it correctly answers the question). Return in JSON format.
    ...
    async def _evaluate_sample(self, sample):
        ...
        # Extract token usage from other_info
        token_usage = {}
        if other_info:
            if isinstance(other_info, dict):
                token_usage = other_info.get("token_usage", {})
            elif isinstance(other_info, list) and len(other_info) > 0:
                # For hierarchical method, other_info is a list of dicts
                # Get token usage from the last item in the list
                last_info = other_info[-1]
                if isinstance(last_info, dict):
                    token_usage = last_info.get("token_usage", {})
        ...
        self.timing_log['summary']['total_prompt_tokens'] += token_usage.get('total_prompt_tokens', 0)
        self.timing_log['summary']['total_completion_tokens'] += token_usage.get('total_completion_tokens', 0)
        self.timing_log['summary']['total_tokens'] += token_usage.get('total_tokens', 0)
        self.timing_log['summary']['total_api_calls'] += token_usage.get('api_calls', 0)
        ...
        self.timing_log['summary']['avg_prompt_tokens'] = self.timing_log['summary']['total_prompt_tokens'] / self.timing_log['summary']['completed_samples']
        self.timing_log['summary']['avg_completion_tokens'] = self.timing_log['summary']['total_completion_tokens'] / self.timing_log['summary']['completed_samples']
        self.timing_log['summary']['avg_total_tokens'] = self.timing_log['summary']['total_tokens'] / self.timing_log['summary']['completed_samples']
    ...
    def evaluate(self):
        ...
        print(f"Average prompt tokens: {self.timing_log['summary']['avg_prompt_tokens']:.0f}")
        print(f"Average completion tokens: {self.timing_log['summary']['avg_completion_tokens']:.0f}")
        print(f"Average total tokens: {self.timing_log['summary']['avg_total_tokens']:.0f}")
        print(f"Total API calls: {self.timing_log['summary']['total_api_calls']}")
...
def run_evaluator(config, evaluator_id):
    ...
    print(f"Evaluator {evaluator_id} - Avg tokens: {evaluator.timing_log['summary']['avg_total_tokens']:.0f}")
...
if __name__ == "__main__":
    ...
    for result in sorted(results, key=lambda x: x['evaluator_id']):
        ...
        print(f"Evaluator {result['evaluator_id']} (tag: {result['tag']}): {result['accuracy']:.2f}% (Time: {total_time:.2f}s, Avg Search: {avg_search_time:.2f}s, Avg Tokens: {avg_total_tokens:.0f})")
    ...
    if results:
        ...
        # Calculate average search time and token usage across all evaluators
        avg_search_times = []
        avg_token_times = []
        for r in results:
        ...
        summary_data = {
            ...
            'statistics': {
                ...
                'average_tokens_per_sample': overall_avg_tokens,
